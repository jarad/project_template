...
sim = list(target = paste0("sim/sim-",1:nsims,".RData"),
					 prereq = rep("sim.R", nsims),
					 recipe = paste0("Rscript -e"," 'i=",1:nsims,"; source(\"sim.R\")'"))

d = expand.grid(i=1:nsims, method=methods)
inf = list(target = paste0("sim/inf-",d$method,"-",d$i,".RData"),
					 prereq = paste0(d$method, ".R ", "sim/sim-", d$i, ".RData"),
					 recipe = paste0("Rscript -e"," 'i=",d$i,"; source(\"",d$method,".R\")'"))

f = expand.grid(i=1:nsims, method=methods, fig=figs)
sum = list(target = paste0("sim/sum-",f$method,"-",f$fig,"-",f$i,".RData"),
					 prereq = paste0(d$method,"-",f$fig, ".R ", "sim/inf-", f$method, "-", f$i,".RData"), # should be able to use inf$prereq here
